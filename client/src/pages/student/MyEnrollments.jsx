                ...
                <table className="md:table-auto table-fixed w-full overflow-hidden border mt-10">
                    <thead className="text-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-semibold truncate max-sm:hidden">Thời gian</th>
                            <th className="px-4 py-3 font-semibold truncate max-sm:hidden">Đã hoàn thành</th>
                        ...
                        {enrolledCourses.map((course, index) => (
                            <tr key={index} className="border-b border-gray-500/20">
                                ...
                                <td className="px-4 py-3 max-sm:hidden">
                                    ...
                                    <span className='text-xs ml-2'>Bài học</span>
                                </td>
                                <td className="px-4 py-3 max-sm:text-right">
                                    <button onClick={() => navigate('/player/' + course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>
                                        {progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Đã hoàn thành' : 'Đang học'}
                                    </button>
                                </td>
                            </tr>
                        ))}
